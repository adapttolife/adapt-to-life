-- One ClickUp subtask per Givebutter transaction. This table stores only
-- projection state plus exact pre-webhook gift facts needed for historical seed.
...
-- Append-only source change cursor. A donor_gifts row is updated as email
-- delivery advances, so its table rowid alone is not a sufficient watermark.
...
-- Append-only verified accounting evidence. Corrections are new negative reversal
-- events, never edits or deletes, so rowid is a durable projection watermark.
