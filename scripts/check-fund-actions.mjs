...
const production=new URL(base).hostname==='adapttolife.org';
...
try{
 for(const width of [320,390,600,768,1024,1440]){
  ...
  if(production){assert.equal(await p.locator('.staging-payment').count(),0);assert.ok(await p.locator('givebutter-giving-form[campaign="IXM5DR"]').isVisible());}else assert.ok(await p.locator('.staging-payment').isVisible());
