...
for (const v of VARIANTS) {
  ...
  for (const [w, h, tag] of SIZES) {
    ...
    const r = await page.evaluate(() => {
      ...
      ["h1", ".btn", ".eyebrow"].forEach((sel) => {
        const el = document.querySelector(`.hs-copy ${sel}`);
        if (!el) return;
        const b = el.getBoundingClientRect();
      ...
      out.weight = performance.getEntriesByType("resource")
        .filter((e) => /\/images\/hero\//.test(e.name))
...
await rm.goto(`${BASE}/hero-a.html`, { waitUntil: "networkidle" });
await rm.waitForTimeout(300);
const vis = await rm.evaluate(() =>
  [...document.querySelectorAll(".hs-fig, .hs-rise")].every((el) => +getComputedStyle(el).opacity === 1));
