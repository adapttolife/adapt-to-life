...
// photographed hero (wheelie-4, 39KB) plus the mid-page band (wheelie-9,
// 56KB; the band ran wheelie-47 at 115KB, then wheelie-167 at 110KB, before
// landing here — the cap has never had to move for any of them). Measured, not estimated: prod own was 168KB before, the image swap is
...
//
// Lowered 2026-09-05, / own 270 -> 170, and this is the ratchet doing the job
// it was raised for. The band photo changed twice by Alec's eye, not for
// weight, and landed on a 56KB frame where the raise had budgeted 115KB. Two
// consecutive staging runs both measured own 162KB. 170 is that plus the same
// ~5% the homepage was always given for encoder and CF-script variance.
//
// The point of taking it back now: 270 would still pass if someone dropped a
...
const BUDGET = {
  "/": { own: 170, total: 700, reqs: 24, hosts: 4, cls: 0.10 },
