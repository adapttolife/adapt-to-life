...
const CHROME = `<aside id="staging-notice" aria-label="Staging environment"><strong>CONTENT STAGING</strong><span>Homepage: first draft. Other pages: current baseline.</span><span>Forms and payments off. Fundraising figures are a dated snapshot.</span></aside><style>#staging-notice{position:fixed;bottom:0;left:0;right:0;z-index:99999;background:#3B3B46;color:#fff;display:flex;justify-content:center;gap:.4rem 1rem;flex-wrap:wrap;padding:.55rem 1rem;font:12px/1.4 system-ui,sans-serif;box-shadow:0 -1px 0 #ffffff55}body{padding-bottom:76px}#staging-notice strong{letter-spacing:.08em}a[data-staging-disabled]{cursor:not-allowed}.staging-payment{padding:2rem;border:1px solid currentColor;border-radius:12px;font:16px/1.5 system-ui,sans-serif}.staging-payment strong{display:block;margin-bottom:.5rem}</style><script>document.addEventListener('submit',function(e){e.preventDefault();e.stopImmediatePropagation();},true);document.addEventListener('click',function(e){var a=e.target.closest('[data-staging-disabled]');if(a){e.preventDefault();e.stopImmediatePropagation();document.getElementById('staging-notice').setAttribute('role','status');}},true);</script>`;
